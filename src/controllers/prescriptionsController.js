    ...
    try {
        const result = await pool.query(`
            SELECT p.id,
                   ...
                   p.takenow,
                   p.user_id,
                   r.title AS recurrence
            FROM tb_prescriptions p
            JOIN tb_recurrence r ON p.recurrence_id = r.id
        `);
...
const createLembrete = async (req, res) => {
    const {name, hours, recurrence_id, takenow, user_id} = req.body;
    try {
        const result = await pool.query('INSERT INTO tb_prescriptions (name, hours, recurrence_id, takenow, user_id) VALUES ($1, $2, $3, $4, $5) RETURNING *',
            [name, hours, recurrence_id, takenow, user_id])
...
const updateLembrete = async (req, res) => {
    ...
    const {name, hours, recurrence_id, takenow, user_id} = req.body
    try {
        const result = await pool.query('UPDATE tb_prescriptions SET name = $1, hours = $2, recurrence_id = $3, takenow = $4, user_id = $5 WHERE id = $6 RETURNING *',
            [name, hours, recurrence_id, takenow, user_id, id])
            res.status(200).json(result.rows[0])
        } catch (err) {
